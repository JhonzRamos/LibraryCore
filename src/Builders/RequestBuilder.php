...
class RequestBuilder
{
    ...
    private function publish($template)
    {
        ...
        $file = new Files();
        $file->path = app_path('Http' . DIRECTORY_SEPARATOR . 'Requests' . DIRECTORY_SEPARATOR . 'Create' . $this->fileName);
        $file->type = "Request";
        $file->created_at = Carbon::now();
        $file->updated_at = Carbon::now();
        $file->menu_id = $this->menuId;
        $file->filename =  'Create' . $this->fileName;
        $file->save();

        $file = new Files();
        $file->path = app_path('Http' . DIRECTORY_SEPARATOR . 'Requests' . DIRECTORY_SEPARATOR . 'Update' . $this->fileName);
        $file->type = "Request";
        $file->created_at = Carbon::now();
        $file->updated_at = Carbon::now();
        $file->menu_id = $this->menuId;
        $file->filename =  'Update' . $this->fileName;
        $file->save();
    }
